          ...
          {faqs.map((faq, i) => (
            <div
              ...
              className={cn(
                ...
                openIndex === i
                  ? 'border-brand-200 shadow-card border-l-4 border-l-brand-600'
                  : 'border-stone-100'
              )}
            >
              <button
                onClick={() => setOpenIndex(openIndex === i ? null : i)}
                className="w-full flex items-center justify-between p-5 lg:p-6 text-left hover:bg-stone-50/50 transition-colors"
                aria-expanded={openIndex === i}
              >
                <span
                  className={cn(
                    'text-base lg:text-lg font-semibold pr-4 transition-colors',
                    openIndex === i ? 'text-brand-800' : 'text-stone-800'
                  )}
                >
                  {faq.question}
                </span>
