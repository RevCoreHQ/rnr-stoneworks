          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {blogPosts.map((post) => (
              <Link
                key={post.slug}
                href={`/blog/${post.slug}`}
                className="group bg-white rounded-2xl border border-stone-100 shadow-soft hover:shadow-elevated overflow-hidden hover:-translate-y-1 transition-all duration-300"
              >
                <div className="relative h-52 bg-stone-200 overflow-hidden">
                  ...
                  <div className="absolute top-3 left-3">
                    <span className="px-2.5 py-1 bg-brand-700/90 text-white text-xs font-medium rounded-full">
                      {post.category}
                    </span>
                  </div>
                </div>
                <div className="p-6">
                  ...
                  <h2 className="text-xl font-display font-semibold text-stone-900 mb-2 group-hover:text-brand-700 transition-colors leading-snug">
                    {post.title}
                  </h2>
